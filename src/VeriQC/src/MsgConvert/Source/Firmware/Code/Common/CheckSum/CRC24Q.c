...
/** table for CRC-24Q (Qualcomm Corporation)
	polynomial g(x) = x^24 + x^23 + x^18 + x^17 + x^14 + x^11 + x^10 + x^7 + x^6 + x^5 + x^4 + x^3 + x + 1
	polynomial g(x) = (1 + x)(x^23 + x^17 + x^13 + x^12 + x^11 + x^9 + x^8 + x^7 + x^5 + x^3 + 1)
	polynomial MaskWord: 0x00864CFB
	seed 0 */
static const unsigned int s_CRC24QTable[256] = {
...
/**********************************************************************//**
@brief   calculate CRC-24Q parity value
		CRCPoly = 0x00864CFB, using CRC table

@param  pData    [In]  pointer of bit stream, MSB first in each 8-bit
@param  DataSize [In]  number of data stream in Unit of Byte
...
unsigned int CalcCRC24Q(const unsigned char *pData, unsigned int DataSize)
{
	...
	{
		CRC = (CRC << 8) ^ s_CRC24QTable[pData[iData] ^ (unsigned char)(CRC>>16)];
	}
	CRC = (CRC & 0x00ffffff);
	return CRC;
}
