...
**************************************************************************/
const static unsigned int s_CRC32Table[256] =
{
...
/**************************************************************//**
@brief Calculate and return the CRC for a binary buffer

@param pDataBuffer [In] Data Buffer
@param BuffLength [In] Data Buffer Size
...
unsigned int CalculateCRC32(unsigned char * pDataBuffer, int BuffLength)
{
	...
	for(iChar = 0; iChar < BuffLength; iChar++)
	{
		CRC = s_CRC32Table[(CRC ^  pDataBuffer[iChar]) & 0xff] ^ (CRC >> 8);
	}
